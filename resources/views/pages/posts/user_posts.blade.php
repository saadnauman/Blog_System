...
<div class="space-y-4">
    @forelse($posts as $post)
        <div class="bg-white dark:bg-gray-800 rounded-lg shadow px-6 py-4 border border-gray-100 dark:border-gray-700 {{ $post->trashed() ? 'opacity-70' : '' }}">
                ...
                <div>
                    <a href="{{ route('posts.show', $post) }}">
                        <h2 class="font-semibold text-lg text-indigo-700 dark:text-indigo-300">
                            {{ $post->title }}
                            @if($post->trashed())
                                <span class="text-sm text-red-500">(Hidden)</span>
                            @endif
                        </h2>
